...
namespace Portal.Apis.Core.Auth
{
    ...
    public class AccountManager : IAccountManager
    {
        ...
        public async Task<Tuple<bool, string[]>> CreateRoleAsync(Role role, IEnumerable<string> claims)
        {
            ...
            {
                result = await this._roleManager.AddClaimAsync(role, new Claim(Configuration.ClaimTypes.Permission, Permissions.GetByValue(claim)));
        ...
        public async Task<Tuple<bool, string[]>> UpdateRoleAsync(Role role, IEnumerable<string> claims)
        {
            ...
            if (claims != null)
            {
                var roleClaims = (await _roleManager.GetClaimsAsync(role)).Where(c => c.Type == Configuration.ClaimTypes.Permission);
                ...
                if (claimsToAdd.Any())
                {
                    ...
                    {
                        result = await _roleManager.AddClaimAsync(role, new Claim(Configuration.ClaimTypes.Permission, Permissions.GetByValue(claim)));
                        if (!result.Succeeded)
                            return Tuple.Create(false, result.Errors.Select(e => e.Description).ToArray());
                    }
                }
