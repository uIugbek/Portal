...
using DynamicClass = System.Linq.Dynamic.Core.DynamicClassFactory;
...
namespace Kendo.DynamicLinq
{
    public static class QueryableExtensions
    {
        ...
        public static (IQueryable<T>, int, object) ApplyQueryState<T>(this IQueryable<T> queryable, int take, int skip, IEnumerable<Sort> sort, Filter filter, IEnumerable<Aggregator> aggregates)
            where T : class
        {
            // Filter the data first
            queryable = Filter(queryable, filter);
            ...
            return (queryable, total, aggregate);
        }
